...
>
> 校验过的 source of truth(校验日期 2026-06-15,version 0.1.0):`pyproject.toml`(console script `rhizome = "kb.cli:main"`,Python ≥3.11,**runtime 零依赖**)、`src/kb/cli.py`、`src/kb/contract.py`、`src/kb/check.py`、`src/kb/sources.py`、`src/kb/adopt.py`、`src/kb/links.py`、`mermaid-validator/validate-mermaid.mjs`。`tests/` 全绿(175 passed, 2 skipped)。

## 鸟瞰

rhizome 是一个**去中心、文件式知识库的写入端 CLI**:把 Markdown + frontmatter 笔记按契约落进正确的域目录,并在 commit 时把不合契约的拦下。它不存数据、不建索引、不做检索——索引与查询交给外部引擎(Qdrant 等)读这些文件自建。整个包是 `src/kb`(历史包名;对外命令是 `rhizome`),**纯 Python、stdlib only**(`tomllib`/`urllib`/`subprocess`),可选一个 Node sidecar 做 Mermaid 校验。4 个 verb:`new`(写)/ `check`(门禁)/ `domains`(看域树 + 与中央索引对账)/ `adopt`(纳管一个仓)。
...
| 符号 / 文件 | 职责 | 路径 |
|---|---|---|
| `main` / `_build_parser` | argparse 入口;4 个子命令(new/check/domains/adopt)的解析与分发 | `src/kb/cli.py` |
| `run_new` | 纯函数:校验输入 → 定域 → 推 identity → 组装 frontmatter → 落文件(不打印) | `src/kb/cli.py` |
| `_cmd_check` / `_fix_paths` | check 子命令编排:per-file + 仓级守卫 + `--fix` 剥字段 + JSON/文本输出 | `src/kb/cli.py` |
| 契约常量 | `REQUIRED_FIELDS`/`OPTIONAL_FIELDS`/`KINDS`(7 值)/`KILLED_FIELDS`/`DERIVED_FIELDS`/`STATUS_ALLOWED`/`ASSET_PREFIXES` | `src/kb/contract.py` |
| `validate_*` | topic(kebab slug)/description(单行)/keywords/kind/assets 校验 | `src/kb/contract.py` |
| `find_repo_root` / `repo_name` | 最近 `.git` 祖先;worktree 下从 `gitdir:` 反推**主 checkout** 名(identity 稳定) | `src/kb/contract.py` |
| `find_domain_dir` / `has_index` | 最近含 INDEX.md 的祖先;`has_index` 用 `os.listdir` 精确比名(APFS 大小写安全) | `src/kb/contract.py` |
| `derive_domain` | **内部物理定位器**:域目录的仓相对路径(只用于落文件,不外泄语义) | `src/kb/contract.py` |
| `derive_node_chain_domain` | **canonical 域口径(C2 node-chain)**:仓根往下、只取自带 INDEX.md 的段拼成域路径 | `src/kb/contract.py` |
| `derive_identity` | `<repo>:<domain>:<slug>`,全位置推导,无 uuid | `src/kb/contract.py` |
| `render_frontmatter` / `render_note` | 组装合规 frontmatter(字段定序 + YAML 安全引用);不注入 H1 | `src/kb/contract.py` |
| `parse_frontmatter` / `split_frontmatter` | 故意做小的扁平 reader(flow/block list、block scalar、quoted/bare、null);非通用 YAML 引擎 | `src/kb/contract.py` |
| `strip_fields` | `--fix` 用:逐行剥指定 top-level key(含续行),正文与其余字节级保留 | `src/kb/contract.py` |
| `is_note_location` / `is_frozen_fm` | 是否在某 KB 域内(域感知 gating);是否冻结(`status: frozen` 或 `kind: decision`) | `src/kb/contract.py` |
| `check_text` | 单篇 raw text 校验,产 `Finding[]`(ERROR 阻断 / WARN 浮现) | `src/kb/check.py` |
| `check_path` | `check_text` + `frozen_gate_findings` + `links.link_findings` | `src/kb/check.py` |
| `frozen_gate_findings` / `head_frozen` | 工作区版本 ≠ **HEAD 冻结版本** → ERROR(`git show HEAD:<rel>`) | `src/kb/check.py` |
| `staged_frozen_findings` | 仓级:staged 的删除/重命名命中 HEAD 冻结 KB 笔记 → ERROR | `src/kb/check.py` |
| `duplicate_domain_findings` / `_iter_index_dirs` | 仓级:两个物理 INDEX.md 目录 C2 域路径相撞 → ERROR | `src/kb/check.py` |
| `mermaid_findings` / `mermaid_blocks` | 抽 ```mermaid 块,经 sidecar 校验;node/sidecar 缺失 → ERROR | `src/kb/check.py` |
| `body_asset_ids` | 从正文「触达资产 / Touched Assets」段抽 asset_id(机械、窄) | `src/kb/check.py` |
| `link_findings` | links(同仓 slug 须解析:断链 ERROR / 跨仓 WARN / identity 形 ERROR)、code(出处 hint,一律 ≤WARN) | `src/kb/links.py` |
| `find_registry` / `load_sources` | 定位 `kb-sources.toml`(4 级查找)+ 解析成 `[(name, path)]` | `src/kb/sources.py` |
| `discover_domains` / `note_domain` | 走 INDEX.md 自发现域树(C2 口径);一篇笔记归哪个域 | `src/kb/sources.py` |
| `build_tree` / `diff` | 域树(喂 surface-hook/recall)/ 与中央 Qdrant 集合的覆盖对账 | `src/kb/sources.py` |
| `central_note_index` / `_qdrant_scroll_page` | scroll 中央集合 → `{repo: {identity: source_path}}`(stdlib urllib);不可达则**大声报错** | `src/kb/sources.py` |
| `run_adopt` | plan-then-apply 幂等纳管:registry 行 + INDEX 骨架(仅当无域)+ lefthook 门禁 | `src/kb/adopt.py` |
| `LEFTHOOK_YML` | 写入仓的 pre-commit 模板:`rhizome check {staged_files}` + `--duplicate-domains --staged-frozen` | `src/kb/adopt.py` |
